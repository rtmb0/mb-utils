...
export function formatSizeWithSeparator(sizeValue: string | string[], separator?: string) {
  const size = Size.create(sizeValue);
  const formatter = new SizeFormatter(size);
  return formatter.formatSizeWithSeparator(separator);
}

export function formatSizesToHTML(sizeValue: string | string[], extraSize?: string, separator?: string) {
  const size = Size.create(sizeValue);
